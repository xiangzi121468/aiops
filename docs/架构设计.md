...
### 3.5.1 可自进化记忆与策略体系（对齐 Evolver 思路）
目标：把“记忆与策略的演化”做成可审计资产，而不是即时修改。

核心组件：
* **Signal Extractor**：从运行日志、评估结果、用户反馈中抽取信号。
* **Gene/Capsule Store**：沉淀可复用的演化资产（策略片段/模板/约束）。
* **Evolve Planner**：选择合适的 Gene/Capsule，生成演化提案。
* **Evolution Events**：记录演化历史（可审计、可回放）。

演化流程：
1. 采集信号（错误率、低命中、用户拒绝、性能退化）。
2. 选择 Gene/Capsule（优先复用已验证资产）。
3. 生成“演化提案”（不直接写核心策略）。
4. 人工审批/自动审批后，进入策略更新/记忆规则更新。
5. 记录 EvolutionEvent（版本、原因、影响范围）。

安全边界：
* **禁止覆盖核心规则文件**（Protected Files）
* **审批与审计强制**（高风险演化必须审批）
* **验证命令白名单**（仅允许安全验证任务）
